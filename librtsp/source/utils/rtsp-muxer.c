...
static void rtsp_muxer_bsf_onpacket(void* param, int64_t pts, int64_t dts, const uint8_t* data, int bytes, int flags)
{
    struct rtp_muxer_media_t* m;
    m = (struct rtp_muxer_media_t*)param;
    m->input(m, flags, pts, dts, data, bytes);
}
...
int rtsp_muxer_sync_timeline(struct rtsp_muxer_t* muxer, int pid, int64_t dts_ms, uint32_t rtp_ts)
{
    struct rtp_muxer_payload_t* pt;
    int64_t pts_ticks;

    if (!muxer || pid < 0 || pid >= muxer->payload_count)
        return -ENOENT;
    pt = &muxer->payloads[pid];
    if (dts_ms < 0)
        dts_ms = 0;
    pts_ticks = dts_ms * pt->rtp.frequency / 1000;
    pt->timeline.rtp = rtp_ts;
    pt->timeline.pts = pts_ticks;
    return 0;
}
