...
static void webm_vpx_codec_configuration_record_fixup(struct webm_vpx_t* vpx)
{
    if (!vpx)
        return;
    /* vpcC 中 0 表示 reserved，ffplay/libav 无法推断色彩空间会导致 swscaler 失败 */
    if (vpx->colour_primaries == 0)
        vpx->colour_primaries = 1; /* BT.709 */
    if (vpx->transfer_characteristics == 0)
        vpx->transfer_characteristics = 1; /* BT.709 */
    if (vpx->matrix_coefficients == 0)
        vpx->matrix_coefficients = 1; /* BT.709 */
    if (vpx->chroma_subsampling == 0 && vpx->profile != 1 && vpx->profile != 3)
        vpx->chroma_subsampling = 1; /* 4:2:0 colocated with luma (0,0) */
}
...
int webm_vpx_codec_configuration_record_from_vp8(struct webm_vpx_t* vpx, int *width, int* height, const void* keyframe, size_t bytes)
{
    ...
    *height = ((uint16_t)(p[9] & 0x3F) << 8) | (uint16_t)(p[8]); // (2 bits Vertical Scale << 14) | Height (14 bits)
    ...
    vpx->level = 31;
    vpx->bit_depth = 8;
    webm_vpx_codec_configuration_record_fixup(vpx);
    return 0;
}
