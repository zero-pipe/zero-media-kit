...
static int mov_read_audio(struct mov_t* mov, struct mov_sample_entry_t* entry)
{
	...
	else if (2 == qtver && box.size >= 36)
	{
		...
		rate_bits = mov_buffer_r64(&mov->io); // audioSampleRate (IEEE754)
		memcpy(&sample_rate_hz, &rate_bits, sizeof(sample_rate_hz));
		channels = mov_buffer_r32(&mov->io); // numAudioChannels
		...
		box.size -= 36;
		if (sample_rate_hz >= 8000.0 && sample_rate_hz <= 384000.0)
			entry->u.audio.samplerate = (uint32_t)(sample_rate_hz * 65536.0);
		if (channels > 0 && channels <= 8)
			entry->u.audio.channelcount = (uint16_t)channels;
	}
