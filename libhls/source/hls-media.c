...
static void* hls_ts_alloc(void* param, size_t bytes)
{
	...
	{
		size_t grow = bytes + N_TS_PACKET * 10 * 1024;
		void* p = realloc(hls->ptr, hls->capacity + grow);
		if (NULL == p)
			return NULL;
		hls->ptr = p;
		hls->capacity += grow;
...
int hls_media_input(struct hls_media_t* hls, int avtype, const void* data, size_t bytes, int64_t pts, int64_t dts, int flags)
{
	...
	/* Forward DTS gap (GOP > segment duration) must not abort; start a new segment instead. */
	force_new_segment = 0;
	if (PTS_NO_VALUE != hls->dts_last && hls->duration > 0 && dts >= hls->dts_last + hls->duration)
		force_new_segment = 1;
