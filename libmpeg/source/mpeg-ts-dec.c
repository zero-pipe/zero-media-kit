...
static void ts_pes_discard_partial(struct pes_t *pes)
{
	if (!pes)
		return;
	pes->pkt.size = 0;
	pes->pkt.vcl = 0;
	pes->have_pes_header = 0;
	pes->len = 0;
}

static void ts_pes_update_cc(struct pes_t *pes, const struct ts_packet_header_t *pkhd)
{
	uint8_t cc;

	if (!pes || !pkhd)
		return;
	cc = (uint8_t)pkhd->continuity_counter;
	if (pkhd->adaptation.discontinuity_indicator) {
		pes->cc = cc;
		pes->cc_valid = 1;
		ts_pes_discard_partial(pes);
		return;
	}
	if (!pes->cc_valid) {
		pes->cc = cc;
	...
	if (((pes->cc + 1) & 0x0F) != cc) {
		uint8_t old_cc = pes->cc;
		uint8_t expected = (old_cc + 1) & 0x0F;
		unsigned forward;

		pes->cc = cc;
		pes->cc_valid = 1;
		if (cc == old_cc)
			return; /* duplicate TS packet */
		forward = (unsigned)((cc - expected) & 0x0F);
		/* Forward CC skip (1..7): likely missing TS payload(s); drop partial PES only then. */
		if (forward >= 1 && forward <= 7 && pes->pkt.size > 0)
			ts_pes_discard_partial(pes);
		return;
	}
	pes->cc = cc;
